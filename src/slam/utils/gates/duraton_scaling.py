...
from slam.utils.gates.bare_candidates import filename, get_group_name
...
def atomic_cost_scaling(
    ...
    if scaled_gate is None:
        ...
        def mid_sl(x):
            return 0.5 * (
                -2 * c
                + np.sqrt(
                    4 * c**2 - 8 * c * x + 4 * c * np.pi - 4 * x**2 + np.pi**2
                )
            )

        def squared_sl(x):
            return np.sqrt((np.pi / 2) ** 2 - x**2)
    ...
    if family_extension:
        ...
        template = MixedOrderBasisCircuitTemplate(
            base_gates=[basis], chatty_build=False, use_smush_polytope=use_smush
        )
        ...
        if metric == 0:
            raise NotImplementedError(
                "Famextend scaling not implemented for haar, calculate it by hand lol"
            )
        if metric == 1:
        ...
        for score_index, gate_target in enumerate(targets):
            ret = recursive_sibling_check(
                template, gate_target, cost_1q=duration_1q, basis_factor=gate.cost()
            )
            if len(targets) == 1:
                return gate, ret[1]
            else:
                scaled_scores[score_index + 1] = ret[
                    1
                ]  # add 1 to index to skip over Haar score
        if len(targets) == 1:
...
def cost_scaling(
    ...
    with h5py.File(filename, "a") as hf:
        ...
        for v in g.values():
            ...
            if family_extension:
                logging.warning(
                    "Family Extension only covers CNOT, B, and SWAP family gates"
                )
            if family_extension and (params[2] == 0 or params[3] == 0):
                # iswap family
                pass_flag = True
            elif family_extension and (
                params[2] / params[3] == 3 or params[3] / params[2] == 3
            ):
                # cnot family
                pass_flag = True
