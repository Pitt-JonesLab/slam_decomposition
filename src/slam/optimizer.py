...
from slam.cost_function import (
    BasicCostInverse,
    EntanglementCostFunction,
    LineSegmentDistanceCost,
    UnitaryCostFunction,
)
...
class TemplateOptimizer:
    def __init__(
        self,
        basis: VariationalTemplate,
        objective: UnitaryCostFunction,
        use_callback=False,
        override_fail=False,
        success_threshold=None,
        training_restarts=None,
        ...
        self.coordinate_list = (
            []
        )  # 2d list sample_iter -> [training iter -> (coordinate)]
        ...
        assert not (
            self.preseeding and self.override_fail
        )  # don't want to save failed data
        assert not (
            self.preseeding and self.basis.n_qubits != 2
        )  # currently preseeding based on weyl coordinates
    ...
    def approximate_target_U(self, target_U):
        ...
        if best_result <= self.success_threshold:
            ...
            if self.basis.n_qubits == 2:
                ...
                logging.info(
                    f"Success: {target_coordinates}, Found: {alternative_coordinate}"
                )
        else:
            if not self.override_fail:
                raise ValueError(
                    "Failed to converge within error threshold. Try increasing restart attempts or increasing temperature scaling on preseed."
                )
            # label target coordinate as fail, label alternative coordinate as succss
            success_label = 0

            # reset back to best size
            if isinstance(self.basis, CircuitTemplate) or isinstance(
            ...
            if self.basis.n_qubits == 2:
                alternative_coordinate = c1c2c3(self.basis.eval(best_Xk))
                logging.info(
                    f"Fail: {target_coordinates}, Found: {alternative_coordinate}"
                )
            if self.preseeding:
                self.basis.data_dict[alternative_coordinate] = DataDictEntry(
                    1, 0, best_Xk, best_cycles
                )
    ...
    def _initialize_run(self, target_U, target_coordinates=None):
        ...
        if self.preseeding and self.basis.coordinate_tree is not None:
            ...
            if (
                found_saved.success_label
            ):  # and found_saved.cycles == target_spanning_range[0]:
                if distance == 0:
    ...
    def _run(self, target_u, target_spanning_range):
        ...
        def objective_func(xk):
            if isinstance(self.objective, UnitaryCostFunction):
                ...
                objf_val = (
                    self.objective.unitary_fidelity(current_u, target_u)
                    / self.objective.normalization
                )
        ...
        def callbackF(xk):
            ...
            if (
                self.basis.n_qubits == 2
            ):  # will break for hamiltonianvariationalobjects?
        ...
        for spanning_iter in target_spanning_range:
            ...
            if isinstance(self.basis, CircuitTemplate) or isinstance(
                ...
                logging.warning(
                    "CircuitTemplate type checking failed OR is HamiltonianTemplate(?)"
                )
            ...
            for r_i in tqdm(range(self.training_restarts)):
                ...
                if best_result < self.success_threshold or (
                    self.override_fail and r_i == self.training_restarts - 1
                ):
